...
class Listener_Encoder(nn.Module):
    def __init__(self, batch_size, attention_size):
        ...
        self.p_bilstm_a = WeightDrop(nn.LSTM(input_size=256,
                                hidden_size=64,
                                num_layers=1,
                                bidirectional=True,
                                batch_first=True),
        ...
        self.p_bilstm_b = WeightDrop(nn.LSTM(input_size=256,
                                hidden_size=64,
                                num_layers=1,
                                bidirectional=True,
                                batch_first=True),
                                     ['weight_hh_l0', 'weight_hh_l0_reverse'],
                                     dropout=0.1, variational=True)
        self.p_bilstm_c = WeightDrop(nn.LSTM(input_size=256,
                                hidden_size=64,
                                num_layers=1,
                                bidirectional=True,
                                batch_first=True),
        ...
        self.value_projection = nn.Linear(128, self.attention_size)
        self.modules.append(self.value_projection)
        self.key_projection = nn.Linear(128, self.attention_size)
    ...
    def forward(self, frames):
        bilstm_output, _ = self.bilstm(frames)
        ...
        p_bilstm_output_a, _ = self.p_bilstm_a(packed_bilstm_output)
        ...
        p_bilstm_output_b, _ = self.p_bilstm_b(packed_p_bilstm_input_b)
        ...
        p_bilstm_output_c, _ = self.p_bilstm_c(packed_p_bilstm_input_c)
...
class Speller_Decoder(nn.Module):
    ...
    def forward(self, keys, values, output_lengths, transcripts, testing):
        if not testing:
            ...
            transcripts = self.embedding(transcripts).to(self.device)
        else:
            sizes = [keys.size()[0], 250]
            predicted_next_word = transcripts.to(self.device)
        ...
        for timestep in range(sizes[1]):
            ...
            if testing:
                if timestep == 0:
                    hidden_a, cell_a = self.lstm_a(torch.cat((self.embedding(predicted_next_word.to(self.device)).unsqueeze(1), context), 2).squeeze(1))
                else:
                    hidden_a, cell_a = self.lstm_a(torch.cat((self.embedding(predicted_next_word.to(self.device)).unsqueeze(1), context), 2).squeeze(1), (hidden_a, cell_a))
            elif not testing:
                if timestep == 0:
                    hidden_a, cell_a = self.lstm_a(torch.cat((transcripts[:, timestep:timestep + 1], context), 2).squeeze(1))
                elif teacher_forcing_chooser >= self.teacher_forcing_prob:
                    hidden_a, cell_a = self.lstm_a(torch.cat((self.embedding(predicted_next_word.to(self.device)).unsqueeze(1), context), 2).squeeze(1), (hidden_a, cell_a))
                else:
                    hidden_a, cell_a = self.lstm_a(torch.cat((transcripts[:, timestep:timestep + 1], context), 2).squeeze(1), (hidden_a, cell_a))
            ...
            # if float('%.3f' % (
            # masked_attention[np.random.randint(0, masked_attention.size()[0] - 1)].sum().item())) != 1:
            ...
            if not testing:
                ...
                predicted_next_word = torch.argmax(noisy_logits, dim=1)
            else:
                predicted_next_word = torch.argmax(logits, dim=1).type(torch.LongTensor)

            output_logits[:, timestep] = logits

            # project from lstm_output to query
            query = self.query_linear(hidden_b)
...
def plot_grad_flow(named_parameters, gradient_path, epoch_num, batch_num):
    ...
    for n, p in named_parameters:
        if (p.requires_grad) and ("bias" not in n):
            ...
            # print(n, "max grad", p.grad.abs().max())
